...
float integralError = 0.0; // Intégrale des erreurs
float lastError = 0.0;
...
int lastUpdatedDutycycleTurn = 11;


float maxIntegralError = 100.0; // Limite pour éviter l'effet "windup"
int angle = 0;
float angle_min = -45;
float angle_max = 45;
volatile int duty_max =  13U;
volatile int duty_min = 8U;
...
//moteur.h
void turn(int erreur){
	// Calcul de l'intégrale de l'erreur
	integralError += erreur;
	float derivativeError = erreur - lastError;
	lastError = erreur; // mise à jour pour la prochaine itération

	// Limiter l'intégrale pour éviter l'effet "windup"
	...
	lastUpdatedDutycycleTurn = updatedDutycycleTurn;
}
...
void detectTrack(int *pixels, int length, int *leftPosition, int *rightPosition) {
    ...
    //int thirdBlackLineStart = -1;
    //int fourthBlackLineStart = 1;
    //int count = 0;
    const int MIN_DISTANCE_BETWEEN_LINES = 40; // Minimum distance between two detected black lines
    const int LEFT_THRESHOLD = 63;// Threshold to determine if the robot is more to the left
    const int RIGHT_THRESHOLD = 64;
    bool inBlack = 0;
    sum=0;
    bool inWhite = 0;
    ...
    int avgBrightness = som / length;

    // Adaptation dynamique du seuil noir
    int dynamicThreshold = avgBrightness * 0.35; */ // Ajuster ce coefficient si nécessaire
    for (int i = 0; i < length; i++) {
    	//sum+=pixels[i];
    	//delay_us(100);
        if (pixels[i] < THRESHOLD_BLACK) { // Détection du noir
            ...
            if (!inBlack) {
            	if(firstBlackLineStart == -1){
            		firstBlackLineStart = i+SEQUENCE_LENGTH;
            	}
            	...
            	/*else if (secondBlackLineStart != -1) {
            		thirdBlackLineStart = i;
            	}
            	else if (thirdBlackLineStart != -1) {
            		fourthBlackLineStart = i - SEQUENCE_LENGTH + 1;
            	}*/
                inBlack = 1;
           }
        } else {
        	if(!inWhite && inBlack){
        		firstBlackLineStart = i-1;
        		inWhite = 1;
        	}
            //count = 0;
            inBlack=0;
    ...
    if (firstBlackLineStart != -1 && secondBlackLineStart != -1) {
        ...
        *rightPosition = -1;
        ...
        if (firstBlackLineStart != -1) {
            if (pixels[1] < THRESHOLD_BLACK) {
                *leftPosition = firstBlackLineStart;
            } else if (pixels[length-1] < THRESHOLD_BLACK) {
                *rightPosition = firstBlackLineStart;
            }
        }
    }
    PRINTF("\r\nValeur left, right: %i, %i\r\n",*leftPosition, *rightPosition);
}
...
/*void detectTrack(int *pixels, int length, int *leftPosition, int *rightPosition) {
	*leftPosition = -1;
	*rightPosition = -1;
	if (pixels[0] >= THRESHOLD_WHITE && pixels[length-1] < THRESHOLD_WHITE){
		for (int i=0; i<length; i++){
			if(pixels[i]<THRESHOLD_WHITE && *rightPosition == -1){
				*rightPosition = i;
			}
		}
	}
	else if (pixels[0] < THRESHOLD_WHITE && pixels[length-1] >= THRESHOLD_WHITE){
		for (int i=length-1; i>=0; i--){
			if(pixels[i]<THRESHOLD_WHITE && *leftPosition == -1){
				*leftPosition = i;
			}
		}
	}
...
/*
	for (int i = 0; i < 128; i++) {
		PRINTF("%d ", pixelArray[i]);  // Affiche chaque élément
	}
	PRINTF("\n");
*/
	...
	/*for (int i = 0; i < 128; i++) {
				if(leftPosition == -1 && i == 0){
					PRINTF(" || ");
				}
				else if (leftPosition == -1 && i == PIXEL_COUNT){
					PRINTF(" || ");
				}
    	...
    	if(usTicks > 200000){
    	   ...
    	   usTicks = 0;
    	        ...
    	        if(newError){
    				...
    				if(erreurActuel > 6 || erreurActuel < -6){  //si erreur pas acceptable
    					if(erreurActuel > 0){
    						turnRight = true;
